...
const StructuredHistory: React.FC = () => {
  ...
  useEffect(() => {
    // Reset current index if chat changes or language changes
    setCurrentIndex(0);
  }, [chatId, language]);
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto mt-6">
        <div className="bg-red-100 text-red-700 border border-red-300 p-4 rounded-lg text-center">
          {t.alert}
        </div>
      </div>
    );
  ...
  const structuredQA =
    chatHistory?.questions?.map((question: string, index: number) => {
      const userMessage = chatHistory.messages.find(
        (m: any) => m.role === "user" && m.question_index === index
      );
      const assistantMessage = chatHistory.messages.find(
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
      ...
      {structuredQA.length > 0 ? (
        ...
        !isLoading && (
          <div className="min-h-screen flex items-center justify-center">
            <p className="text-gray-600 text-lg">{t.alert}</p>
          </div>
        )
      )}
    </div>
